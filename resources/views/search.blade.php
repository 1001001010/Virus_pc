...
@section('content')
    ...
    <div class="main-container">
        ...
        <div class="search-container">
            <form action="{{ route('search', ['category_id' => $category->id]) }}" method="POST">
                @csrf
                <input type="text" id="search-input" name="word" placeholder="Поиск по моделям...">
        ...
        <div class="tri">
            @foreach ($result as $product)
                <div class="block1">
                    ...
                    <a class="dd" href="{{ route('product', ['product_id' => $product->id]) }}">
                        <p class="pc-one">{{ $product->name }}</p>
                    </a>
